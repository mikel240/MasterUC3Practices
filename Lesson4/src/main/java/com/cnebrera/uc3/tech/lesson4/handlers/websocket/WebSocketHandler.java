...
@WebSocketHandlerService(path = "/prices/websocket", broadcaster = SimpleBroadcaster.class, atmosphereConfig = {
...
public class WebSocketHandler extends WebSocketStreamingHandlerAdapter {
    ...
    public void onOpen(final WebSocket webSocket) {
        ...
        // Add the listener to publishers
            ...
            public void onDisconnect(AtmosphereResourceEvent event) {
                if (event.isCancelled() || event.isClosedByClient() || event.isClosedByApplication()) {
                    if (event.isCancelled()) {
                        WebSocketHandler.LOGGER.warn(
                                "Browser {} unexpectedly disconnected",
